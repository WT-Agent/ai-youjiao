  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">儿童年龄段</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">游戏目标</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">幼教早教游戏与亲子感统训练指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 幼教早教游戏与亲子启蒙专家');
...
const metricsList = [
  {
    "key": "earlyChildhoodPlayfulness",
    "label": "早教游戏趣味性"
  },
  {
    "key": "sensoryIntegrationValue",
    "label": "感统训练价值"
  },
  {
    "key": "parentChildInteractionSafety",
    "label": "亲子互动安全性"
  },
  {
    "key": "cognitive启蒙Impact",
    "label": "认知启蒙促进分"
  },
  {
    "key": "behavioralHabitGuidance",
    "label": "行为习惯引导度"
  }
];
const guideRules = [
  {
    "title": "安全第一防吞咽磕碰",
    "advice": "选择无毒无味大颗粒道具，游戏区域清理锐角尖角，全程保持家长视线监护",
    "avoid": "切忌使用小于 3cm 的小珠子或易碎玻璃道具"
  },
  {
    "title": "游戏过程胜过输赢结果",
    "advice": "多夸奖孩子在游戏中的尝试与坚持（如“你刚才试了三次终于叠高了”），少评判输赢",
    "avoid": "避免在游戏中表现出急躁或代劳"
  },
  {
    "title": "控制单次游戏节奏",
    "advice": "幼童集中精力时间短，单个游戏控制在 10-15 分钟，意犹未尽时结束保持新鲜感",
    "avoid": "严禁强迫孩子重复做同一个游戏导致厌烦"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[YOUJIAO_SCORES\][\s\S]*?\[\/YOUJIAO_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[YOUJIAO_SCORES\](.*?)\[\/YOUJIAO_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('youjiao_history_records');
...
const saveHistory = () => {
  localStorage.setItem('youjiao_history_records', JSON.stringify(historyList.value));
};
